...
function processCallbacks(namedRouteParameters, data, req, res, successHandler
	...
	debug('processing callbacks: %s', namedRouteParameters);
...
function matchRoute(method, url, apiRoutes, params) {
	...
	debug('matched route %s', matchedRoute);
	return matchedRoute;
}
...
function dispatchLocalApiCall(httpMethod, url, data, options) {
	debug('dispatching local api call (method: %s, url: %s)', httpMethod, url);
	var self = this
		, params = {}
...
function sync(method, model, options) {
	debug('syncing %s to %s', method, model);
